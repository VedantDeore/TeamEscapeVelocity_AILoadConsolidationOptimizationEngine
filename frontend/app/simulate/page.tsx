...
import {
  FlaskConical, BarChart3, Award, Zap, ArrowRight
} from 'lucide-react';
import {
  ...
  return (
    <div style={{
      ...
      {payload.map((entry: any, i: number) => (
        <p key={i} style={{ color: entry.fill || entry.stroke, fontWeight: 700 }}>
...
const cardStyles = [
  ...
  { border: 'rgba(99,91,255,0.35)', top: 'linear-gradient(90deg, #635BFF, #8B5CF6)', title: '#635BFF' },
  ...
  return (
    <>
      ...
      <div className="page-body">
        ...
        {/* ── Scenario Cards ── */}
        <div className="scenario-grid stagger-children" style={{ marginBottom: '28px' }}>
            ...
            >
              ...
              {i === 1 && (
                <div style={{
                  ...
                  background: cardStyles[i].top, borderRadius: '12px 12px 0 0',
                }} />
              )}

              <div className="scenario-name" style={{ color: cardStyles[i].title }}>
                {scenario.name}
                {i === 1 && (
                  <span className="badge badge-primary" style={{ marginLeft: '8px', fontSize: '10px' }}>
                    <Award size={10} style={{ marginRight: '3px' }} />Best
                  </span>
                )}
              </div>
              ...
              {[
                ...
                { label: 'Avg Utilization', val: `${scenario.avgUtilization}%` },
                { label: 'Total Cost',     val: `₹${(scenario.totalCost/100000).toFixed(1)}L` },
                { label: 'CO₂ Emissions',  val: `${scenario.co2Emissions.toLocaleString()} kg` },
            ...
            <div className="card-body" style={{ height: '320px' }}>
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart data={radarData}>
                  ...
                  <PolarAngleAxis dataKey="metric" tick={{ fill: '#8792a2', fontSize: 10 }} />
        ...
        <div className="hero-dark-section animate-slide-up">
          ...
          <div className="hero-stats-row">
            {[
              ...
              { value: '+29%',  label: 'Utilization gain',  pct: '50%', cls: 'purple' },
              { value: '₹1.4L', label: 'Cost saved/day',    pct: '31%', cls: 'amber'  },
              { value: '800 kg',label: 'CO₂ reduced',       pct: '33%', cls: 'green'  },
            ].map((s) => (
              <div key={s.label} className="hero-stat">
                <div className={`hero-stat-value ${s.cls}`}>{s.value}</div>
                <div className="hero-stat-label">
                  {s.label}<br />
                  <span style={{ color: '#34d399', fontWeight: 600 }}>▼ {s.pct} improvement</span>
                </div>
              </div>
            ))}
          </div>
